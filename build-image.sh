...
TAG_NAME=bookapi # use whatever tag you'd like for this (e.g. reponame/bookapi, reponame/customname)
...
if [ "$CHIP_TYPE" = "Apple M1 Max" ]; then
  ...
  docker buildx build --platform linux/amd64 -t $TAG_NAME -f Dockerfile .
else
